...
零依赖、零后端的单页应用。全部逻辑在浏览器里跑：法语词形还原、CEFR 分级、易错点检测、SM-2 间隔复习、PDF 文本层抽取、Office 文档解析、扫描件 OCR、**专业词汇的中文释义自动补齐**。**不配置任何东西也能用**（内置免费公共 AI），接了自己的 Key 会更稳更快。
...
设置里默认就是 **免费公共 AI**，不用注册、不用填 Key，选个模型就能用。
解析材料后会自动把缺的中文释义补上（离线词库只有约 400 条核心词，专业词汇必须靠模型）。

代价要说清楚：这是第三方公益推理服务，**时快时慢，偶尔连不上**（失败会自动重试，失败的批次会跳过而不是整轮作废），
而且请求内容会经过对方服务器 —— 日常练习材料没问题，证件、病历、未公开试卷这类别往里传。

### 2. 自己填 Key（要稳定和隐私就选这个）
