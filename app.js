...
/* 到底能不能调用模型：自己有 Key，或者站主配了共享代理且访客选了它 */
function hasLLM() { return !!S.api.key || (hasProxy() && S.api.preset === 'proxy'); }

const DEFAULT_STATE = () => ({
  exam: 'B1', examDate: '', rawText: '',
  vocab: [], grammar: [], cards: [], mode: 'recognize',
  api: { provider: 'claude', base: '', key: '', model: '' },
...
function applyPreset(key, fill) {
  ...
  ['#apiKeyRow', '#apiBaseRow', '#apiModelRow'].forEach(s => {
    const el = $(s);
    if (el) el.classList.toggle('hide', !!p.shared);
  });
  if (!fill) return;
...
/* ---------------- 导入 ---------------- */
{
  ...
  if (sel) {
    ...
    // 老存档没有 preset，按 base 反推，推不出来就当自定义
    let guess = S.api.preset || (hasProxy() ? 'proxy' : 'custom');
    if (!S.api.preset && S.api.base) {
      guess = keys.find(k => API_PRESETS[k].base && S.api.base.indexOf(API_PRESETS[k].base.replace(/^https?:\/\//, '')) >= 0)
        || (hasProxy() ? 'proxy' : 'custom');
    }
...
$('#btnAnalyze').onclick = async () => {
  ...
  const noteHtml = `<div class="note" style="margin-top:12px">
    ...
    下一步：在这张表里勾选要背的词，或者直接点「加入最高频 50 个」。
    ${hasLLM() ? '<br>模型已接入，释义与等级会由模型校准。' : ''}
    ${!hasLLM() ? '<br>提示：释义为空是因为离线词库只有约 400 条核心词。到「设置」接入模型后会自动补齐中文释义与等级校准。' : ''}
  </div>`;
...
function readApiForm() {
  return {
    ...
    key: $('#apiKey').value.trim(), model: $('#apiModel').value.trim()
  };
}
$('#btnSaveApi').onclick = () => {
  S.api = readApiForm();
